...
///   6 1 2
///   7 8 9
let Neighbourhood (image:BinaryImage, i, j) =
...
let Degree neighbourhoodToConnectivity (image:BinaryImage) =
  let pixelDegree i j value = 
    if value then
      Neighbourhood (image, i, j) |> neighbourhoodToConnectivity
    else
      0
  new IntegerImage(Array2.mapi pixelDegree image.Elements)

/// For set (foreground) pixels returns the 8-connectivity
/// degree of the pixel. For unset (background pixels)
/// return 0
let DegreeEight (image:BinaryImage) =
  Degree EightConnectivity image

/// For set (foreground) pixels returns the 8-connectivity
/// degree of the pixel. For unset (background pixels)
/// return 0
let DegreeFour (image:BinaryImage) =
...
let Connectivity (image:BinaryImage) = 
  let connectivity i j =
     if image.[i, j] then
       ...
       0
  new IntegerImage(Array2.init image.SizeI image.SizeJ connectivity)
...
/// component.
let labelPixel (labelMap:IntegerImage) labelCount i j =
  let labelValue p q =
    if labelMap.InRange(p, q) then labelMap.[p, q] else 0
...
let ConnectedComponents (image:BinaryImage) =
  ...
  let labelMap = new IntegerImage(Array2.zero_create image.SizeI image.SizeJ)
  let mutable labelCount = 0
  let mutable equivalences = []
  for j = image.SizeJ - 1 downto 0 do 
    for i = 0 to image.SizeI - 1 do
        if image.[i, j] then
          let (result, equiv) = labelPixel labelMap labelCount i j
          if result > labelCount then
            labelCount <- result
          if List.nonempty equiv then
  ...
  let equivalenceSets = List.map Set.of_list equivalences |> List.filter (fun s -> Set.size s > 1) |> Set.of_seq
  ...
  new IntegerImage(Array2.map labelToComponent labelMap.Elements)
